...
substance = '2mlt'
fixer = PDBFixer(filename= substance + '.pdb')
...
PDBFile.writeFile(fixer.topology, fixer.positions, open(substance + '-fixed.pdb', 'w'))
...
structure = PDBFile(substance + '-fixed.pdb')
...
# actual pdb file.
structure_model = Modeller(structure.topology, structure.positions)

# Add a tip3p waterbox solvent
structure_model.addSolvent(forcefield, model = 'tip3p', padding = 1.5*nanometers)
##################################################################################################################################
...
system = forcefield.createSystem(structure_model.topology, nonbondedMethod = PME, 
...
simulation = Simulation(structure_model.topology, system, integrator, platform)
simulation.context.setPositions(structure_model.positions)
##################################################################################################################################
...
simulation.reporters.append(PDBReporter(substance + '_trajectory.pdb', print_every_Nsteps))
...
traj = md.load(substance + '_trajectory.pdb')
print(traj)
